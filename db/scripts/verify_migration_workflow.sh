...
DATABASE_URL="${database_url}" "${ROOT_DIR}/db/scripts/migrate.sh" up >/dev/null
...
applied_status_count="$(DATABASE_URL="${database_url}" "${ROOT_DIR}/db/scripts/migrate.sh" status | grep -c ' applied$')"
...
backup_dir="$(mktemp -d)"
backup_path="$(DATABASE_URL="${database_url}" BACKUP_DIR="${backup_dir}" "${ROOT_DIR}/db/scripts/backup.sh")"

docker exec -i "${container_name}" dropdb -U cabugi "${temp_db}" >/dev/null
docker exec -i "${container_name}" createdb -U cabugi "${temp_db}" >/dev/null
CONFIRM_RESTORE=yes DATABASE_URL="${database_url}" "${ROOT_DIR}/db/scripts/restore.sh" "${backup_path}" >/dev/null
